...
instance.interceptors.request.use(config => {
    if (config.method === 'POST' || config.method === 'post' || config.method === 'PUT' || config.method === 'put') {
        const ctype = config.headers['Content-Type'] || 'application/x-www-form-urlencoded'
...
instance.interceptors.response.use(response => {
    ...
    if (errno === 0) {
        ...
        let message = typeof (res.errmsg) === 'string' ? res.errmsg : typeof (res.errmsg) === 'object' ? Object.values(res.errmsg)[0][0] : JSON.stringify(res.errmsg)
        app.$message({
            type: 'error',
            message: message
        }) 
    ...
    let res = (error.response.data && JSON.parse(error.response.data)) || {};
    if (res.status === 401) {
    ...
    }else if (res.status === 503) { // 请求频繁，api和nginx
        app.$message({
            type: 'error',
